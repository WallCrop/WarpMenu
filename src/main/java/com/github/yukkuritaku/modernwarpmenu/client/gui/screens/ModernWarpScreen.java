    ...
    private void onChestItemChange(int triggerCount) {
        ...
        if (triggerCount > (this.lastSlotIndexToCheck + 1) * 2) {
            try {
                ...
                // WORKAROUND: For 26.1.2, always show custom UI if menu title matched
                // The item matching may fail due to Hypixel updates, but if the title is correct, we should show the UI
                boolean showCustomUI = menuItemsMatch || true; // Always true for now
                setCustomUIState(showCustomUI, showCustomUI);
                updateButtonStates();
                if (this.configButton != null) {
                    this.configButton.setVisible(showCustomUI);
                }
                if (!menuItemsMatch) {
                    // Reduced to debug level - don't spam chat
                    LOGGER.debug("Warning: Chest has correct name but items mismatched");
                }
